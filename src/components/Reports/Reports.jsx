  ...
  return (
    <>
      ...
      <div className="container mx-auto mt-6 p-4 bg-zinc-800 text-white rounded-md">
        ...
        <div className="flex p-3 flex-col md:flex-row gap-10">
              ...
              <table className="min-w-full text-white rounded-md overflow-hidden text-sm md:text-md">
                ...
                <tbody>
                  {expenses.map((expense) => (
                    ...
                    >
                      <td className="py-4 px-2 lg:px-6">{expense.description}</td>
                      <td className="py-4 px-2 lg:px-6">{expense.amount}</td>
                      <td className="py-4 px-2 lg:px-6">{expense.category}</td>
                      <td className="py-4 px-2 lg:px-6">{expense.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <p className="my-3 font-bold text-zinc-300">Total Value : {expenses.reduce((acc, curr) => acc + curr.amount, 0)}</p>
              </>
            ) : (
